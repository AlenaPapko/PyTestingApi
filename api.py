...
class Pets:
    ...
    def get_token(self) -> json:
        """Запрос к Swagger сайта для получения токена"""
    ...
    def create_pet(self) -> json:
        ...
        headers = {'Authorization': f'Bearer {my_token}'}
        data = {"id": 0,
                "name": 'Ruffu', "type": 'dog', "age": 9, "owner_id": my_id}
    ...
    def upload_pet_photo(self):
        """Запрос к Swagger сайта для загрузки фотографии к аккаунту созданного питомца зарегистрированным пользователем"""
        my_token = Pets().get_token()[0]
        pet_id = Pets().create_pet()[0]
        headers = {'Authorization': f'Bearer {my_token}'}
        files = {'pic': ('Ruffy.jpg', open('tests\\Photo\\Ruffy.jpg', 'rb'), 'image/jpg')}
        #pic = open('tests\\Photo\\Ruffy.jpg', 'rb')
        #files = {pic: pic}
    ...
    def update_pet(self) -> json:
        ...
        pet_id = Pets().post_pet()[1]
        headers = {'Authorization': f'Bearer {my_token}'}
        data = {"id": pet_id, "type": "dog", "name": 'Zhora', "owner_id": my_id}
    ...
    def update_pet_neg(self) -> json:
        ...
        pet_id = 1547
        headers = {'Authorization': f'Bearer {my_token}'}
        data = {"id": pet_id, "type": "dog", "name": 'Zhoraa', "owner_id": my_id}
        res = requests.patch(self.base_url + 'pet', data=json.dumps(data), headers=headers)
        status = res.status_code
        return status
    ...
    def create_pet_neg(self) -> json:
        ...
        headers = {'Authorization': f'Bearer {my_token}'}
        data = {"id": 0,
                "name": 'Ruffy', "type": 'dog', "age": 9, "owner_id": my_id}
        res = requests.post(self.base_url + 'pet', data=json.dumps(data), headers=headers)
        status = res.status_code
        return status
